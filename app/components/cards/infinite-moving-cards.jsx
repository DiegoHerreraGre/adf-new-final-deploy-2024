...
export const InfiniteMovingCards = ({
  ...
  return (
    ...
    >
      <ul
        ref={scrollerRef}
        className={`flex min-w-full shrink-0 gap-10 py-4 w-max flex-nowrap ${start && "animate-scroll "} ${pauseOnHover && "hover:[animation-play-state:paused]"}`}
      >
        {items.map((item) => (
          <li
            className="w-[500px] max-w-full relative rounded-2xl border border-b-0 flex-shrink-0 border-slate-700 px-8 py-6 md:w-[450px]"
            style={{
              background:
